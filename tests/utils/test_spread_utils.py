...
class TestCalculateSpread:
    ...
    def test_calculate_spread_with_invalid_min_ask_value_fails(self):
        ...
        assert "Error calculating spread for market market_2" in str(excinfo.value)
    ...
    def test_calculate_spread_with_invalid_max_bid_value_fails(self):
        ...
        with pytest.raises(ValueError) as excinfo:
            calculate_spread(ticker)
        assert "Error calculating spread for market market_3" in str(excinfo.value)
    ...
    def test_calculate_spread_with_invalid_min_ask_and_max_bid_values_fails(self):
        ...
        with pytest.raises(ValueError) as excinfo:
            calculate_spread(ticker)
        assert "Error calculating spread for market market_4" in str(excinfo.value)
...
class TestCompareSpreadWithAlertValue:
    def test_compare_spread_with_greater_spread_value_than_alert_value_succeeds(self):
        ...
        result = compare_spread_with_alert_value("10.0", "5.0", "market_1")
        assert result["market_id"] == "market_1"
        assert result["spread_value"] == "10.00"
        assert result["alert_value"] == "5.00"
        assert result["is_greater"] is True
        assert result["is_less"] is False
        assert (
            result["message"]
            == "Spread for market market_1 is GREATER than the alert value by 5.00. Spread Value: 10.00, Alert Value: 5.00"
        )
    ...
    def test_compare_spread_with_lesser_spread_value_than_alert_value_succeeds(self):
        ...
        result = compare_spread_with_alert_value("5.0", "10.0", "market_2")
        assert result["market_id"] == "market_2"
        assert result["spread_value"] == "5.00"
        assert result["alert_value"] == "10.00"
        assert result["is_greater"] is False
        assert result["is_less"] is True
        assert (
            result["message"]
            == "Spread for market market_2 is LESS than the alert value by 5.00. Spread Value: 5.00, Alert Value: 10.00"
        )
    ...
    def test_compare_spread_with_equal_spread_value_than_alert_value_succeeds(self):
        ...
        result = compare_spread_with_alert_value("7.0", "7.0", "market_3")
        assert result["market_id"] == "market_3"
        assert result["spread_value"] == "7.00"
        assert result["alert_value"] == "7.00"
        assert result["is_greater"] is False
        assert result["is_less"] is False
        assert (
            result["message"]
            == "Spread is for market market_3 is EQUAL to the alert value. Spread Value: 7.00, Alert Value: 7.00"
        )

    def test_compare_spread_with_invalid_spread_value_fails(self):
        # Test with an invalid spread_value
        with pytest.raises(ValueError) as excinfo:
            compare_spread_with_alert_value("invalid", "5.0", "market_1")
        assert "Error formatting spread for market market_1" in str(excinfo.value)

    def test_compare_spread_with_invalid_alert_value_fails(self):
        # Test with an invalid alert_value
        with pytest.raises(ValueError) as excinfo:
            compare_spread_with_alert_value("10.0", "invalid", "market_2")
        assert "Error formatting spread for market market_2" in str(excinfo.value)
    ...
    def test_compare_spread_with_invalid_spread_and_alert_values_fails(self):
        ...
        with pytest.raises(ValueError) as excinfo:
            compare_spread_with_alert_value("invalid", "also_invalid", "market_3")
        assert "Error formatting spread for market market_3" in str(excinfo.value)
